# 哈基米64 (Hachi64) - Dart 实现

Dart 实现的哈基米64编解码器，使用64个中文字符进行 Base64 风格的编码和解码。适用于 Flutter 和 Dart Web 应用。
...
**重要提示：** 使用哈基米字母表会使您编码的数据与标准 Base64 解码器不兼容。仅在您完全控制编码和解码过程时才使用此功能。

## 常见问题

### Q: 为什么编码结果全是中文？
A: 这正是哈基米64的特色！它使用64个精心挑选的中文字符代替标准 Base64 的字符集，使编码结果看起来像是有意义的中文文本。

### Q: 可以和标准 Base64 互相转换吗？
A: 不能直接转换。哈基米64使用的是自定义字符集，与标准 Base64 不兼容。
